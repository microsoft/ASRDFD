...
stop_filtering_volume(char *uuid, inm_devhandle_t *idhp, int dbs_flag)
{
	...
	if (driver_ctx->dc_root_disk == tgt_ctxt) {
		...
		err("stop_filtering_volume, setting dc_root_disk to NULL");
	}
...
inm_s32_t remove_filter_device(char *uuid)
{
	...
	if (driver_ctx->dc_root_disk == tgt_ctxt) {
		driver_ctx->dc_root_disk = NULL;
		err("remove_filter_device, setting dc_root_disk to NULL");
	}
						...
						void __INM_USER *arg)
{
	...
	if (driver_ctx->dc_root_disk == tgt_ctxt) {
		driver_ctx->dc_root_disk = NULL;
		err("process_volume_unstacking_ioctl, setting dc_root_disk to NULL");
	}
...
#ifdef INM_QUEUE_RQ_ENABLED
		len += snprintf(page+len, (INM_PAGESZ - len),
			"Write BIOs via queue_rq             : %d\n",
			INM_ATOMIC_READ(&ctxt->tc_nr_queue_rq_bios));
#endif

#ifdef INM_QUEUE_RQS_ENABLED
		len += snprintf(page+len, (INM_PAGESZ - len),
			"Write BIOs via queue_rqs            : %d\n",
			INM_ATOMIC_READ(&ctxt->tc_nr_queue_rqs_bios));
#endif
...
#ifdef INM_QUEUE_RQ_ENABLED
	vol_statsp->ullNrQueueRqBios = INM_ATOMIC_READ(&tgt_ctxt->tc_nr_queue_rq_bios);
#ifdef INM_QUEUE_RQS_ENABLED
	vol_statsp->ullNrQueueRqsBios = INM_ATOMIC_READ(&tgt_ctxt->tc_nr_queue_rqs_bios);
#endif
#endif
...
process_tag_volume_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
	...
	*/
	...
	for(numvol = 0; numvol < tag_vol->nr_vols; numvol++) {
		...
		if (tag_vol->vol_info->flags & TAG_DISK_NO_DRAIN_BARRIER) {
...
static void dump_target_context(target_context_t *tgt)
{
    if (!tgt) return;
    info("Target Context: %p", tgt);
    info("  GUID: %s", tgt->tc_guid);
    info("  Name: %s", tgt->tc_pname);
    info("  Flags: 0x%x", tgt->tc_flags);
    // Add more fields as needed
}

static void dump_driver_ctx(void)
{
    info("Driver Context: %p", driver_ctx);
    info("  Root Disk: %p", driver_ctx->dc_root_disk);
    info("  Root Dev: 0x%x", driver_ctx->root_dev);
    // Add more fields as needed

    // Dump all target contexts
    target_context_t *tgt;
    INM_DOWN_READ(&driver_ctx->tgt_list_sem);
    list_for_each_entry(tgt, &driver_ctx->tgt_list, tc_list) {
        dump_target_context(tgt);
    }
    INM_UP_READ(&driver_ctx->tgt_list_sem);
}

inm_s32_t process_dump_driver_structs_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
    info("=== Dumping all driver data structures ===");
    dump_driver_ctx();
    info("=== End dump ===");
    return 0;
}

inm_s32_t process_dbg_as_err_ioctl(inm_devhandle_t *idhp, void __INM_USER *arg)
{
    int enable = 0;
    if (copy_from_user(&enable, arg, sizeof(int)))
        return -EFAULT;
    g_dbg_as_err = enable ? 1 : 0;
    info("dbg logs will now be sent to %s", g_dbg_as_err ? "err" : "dbg");
    return 0;
}
