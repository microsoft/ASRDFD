...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 5, 0) || defined(RHEL9_4) || defined(RHEL9_5) || defined(RHEL9_6) || defined(SLES15SP6) || defined(SLES15SP7)
static int
...
#if defined(INM_HANDLE_FOR_BDEV_ENABLED) || defined(INM_FILP_FOR_BDEV_ENABLED)
inm_s32_t
is_rootfs_ro(void)
{
	int retval = 0;
	...
	filp = inm_file_open_by_devnum(driver_ctx->root_dev, FMODE_READ);
	if (IS_ERR(filp))
		return 0;
	bdevp = file_bdev(filp);
#endif
	...
	*hdcp = (host_dev_ctx_t *) INM_KMALLOC(sizeof(host_dev_ctx_t),
	...
	switch (dev_info->d_type) {
		...
		if (handle) {
			bdev = handle->bdev;
#elif defined(INM_FILP_FOR_BDEV_ENABLED)
		filp = inm_file_open_by_dev_path(dev_info->d_guid, FMODE_READ);
		if (filp) {
			bdev = file_bdev(filp);
#else
		...
		if (bdev) {
			...
			if (hdc_dev) {
				...
				hdc_dev->hdc_dev = bd_inode->i_rdev;
#else
...
#elif defined(INM_FILP_FOR_BDEV_ENABLED)
struct file *
inm_file_open_by_dev_path(char *path, int mode)
{
	inm_dev_t dev = 0;
	struct file *filp;
	...
	filp = bdev_file_open_by_path(path, mode, NULL, NULL);
	if (IS_ERR(filp))
		return NULL;
	...
	case FILTER_DEV_MIRROR_SETUP:
		...
		if (hdcp) {
			...
			if (handle) {
				...
				close_bdev_handle(handle);
#elif defined(INM_FILP_FOR_BDEV_ENABLED)
			filp = inm_file_open_by_dev_path(ctx->tc_guid, FMODE_READ);
			if (filp) {
				bdev = file_bdev(filp);
				devid = bdev->bd_inode->i_rdev;
				close_file(filp);
...
inm_get_dev_t_from_path(const char *pathp)
{
	...
	if (handle) {
		...
		close_bdev_handle(handle);
#elif defined(INM_FILP_FOR_BDEV_ENABLED)
	filp = inm_file_open_by_dev_path((char *)pathp, FMODE_READ);
	if (filp) {
		bdevp = file_bdev(filp);
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6,10,0)
		bd_inode = filp->f_mapping->host;
#else
		bd_inode = bdevp->bd_inode;
#endif
		rdev = bd_inode->i_rdev;
		close_file(filp);
#else
...
#elif defined(INM_FILP_FOR_BDEV_ENABLED)
struct file *
inm_file_open_by_devnum(dev_t dev, unsigned mode)
{
...
#elif defined(INM_FILP_FOR_BDEV_ENABLED)
			close_file (freeze_ele->filp);
			freeze_ele->filp = NULL;
#else
